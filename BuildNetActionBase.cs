...
namespace Inedo.BuildMasterExtensions.DotNet2
{
    ...
    /// </summary>
    public abstract class BuildNetActionBase : CommandLineActionBase
    {
        /// <summary>
        ...
        /// <summary>
        /// Initializes a new instance of the <see cref="BuildNetActionBase"/> class.
        /// </summary>
        protected BuildNetActionBase()
        {
        }
        ...
        {
            var frameworkPath = ((DotNetConfigurer)this.GetExtensionConfigurer()).FrameworkRuntimePath;
        ...
        protected int InvokeMSBuild(string arguments, string workingDirectory)
        {
            try
            {
                ...
                try
                {
                    ...
                    logFile = Path.Combine(this.RemoteConfiguration.TempDirectory, Guid.NewGuid().ToString("N"));
                ...
                if (!useConsoleOutput)
                {
                    try
                    {
                        ...
                        {
                            var level = (MessageLevels)Enum.Parse(typeof(MessageLevels), node.GetAttribute("level"));
        ...
        {
            var assemblyPath = Path.Combine(this.RemoteConfiguration.TempDirectory, "XmlBuildLogger.dll");
            using (var input = typeof(BuildNetActionBase).Assembly.GetManifestResourceStream("Inedo.BuildMasterExtensions.DotNet2.XmlBuildLogger.dll"))
