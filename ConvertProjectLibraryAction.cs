...
namespace Inedo.BuildMasterExtensions.DotNet2
{
    ...
    /// </summary>
    [ActionProperties("Convert Library References", "Converts project references in .NET projects to file references.", ".NET")]
    [CustomEditor(typeof(ConvertProjectLibraryActionEditor))]
    public sealed class ConvertProjectLibraryAction : RemoteActionBase
    {
        /// <summary>
        ...
        /// </summary>
        public ConvertProjectLibraryAction()
        {
        }
        ...
        {
            this.LibraryPath = new AgentHelper().GetWorkingDirectory(this.Context.ApplicationId, this.Context.DeployableId, this.LibraryPath);
            var sourcePath = this.RemoteConfiguration.SourceDirectory;
            Exception[] unused;
            LogDebug("Getting entries at " + sourcePath + "...");
            var entry = Util.Files.GetDirectoryEntry(sourcePath, out unused, this.Recursive);
        ...
        private static AssemblyName GetFullAssemblyName(string assemblyFileName)
        {
            ...
            catch (BadImageFormatException)
            {
                try
                {
                    return MetadataServices.NativeMetadataInspector.GetAssemblyName(assemblyFileName);
                }
                catch (Exception ex)
                {
